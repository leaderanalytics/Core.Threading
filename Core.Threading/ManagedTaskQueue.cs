...
namespace LeaderAnalytics.Core.Threading
{
    public class ManagedTaskQueue
    {
        ...
        public ManagedTaskQueue(int maxThreads)
        {
            ...
            Task.Factory.StartNew(() =>
            {
                while (true)
                {
                    ...
                    throttle.Wait();
                    RaiseTaskCountChanged(_maxThreads - throttle.CurrentCount);
        ...
        private void RaiseTaskCountChanged(int taskCount)
        {
            if (TaskCountChanged != null)
                TaskCountChanged(taskCount);
        }
    }
}
